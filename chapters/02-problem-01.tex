...
\begin{tikzpicture}[
    scale=0.9,
    font=\small,
    number/.style={minimum width=1.2cm, minimum height=0.6cm, anchor=center},
    arrow/.style={->, thick, color=blue!70}
]

% Grid for the decimal expansions
\foreach \i in {0,...,4} {
    \foreach \j in {0,...,6} {
        \node[number] at (\j*1.2, -\i*0.8) {\textcolor{gray!70}{0}};
    }
}

% Function values f(0), f(1), f(2), ...
\node[above] at (-1.2, 0) {$f(0) =$};
\node[above] at (-1.2, -0.8) {$f(1) =$};
\node[above] at (-1.2, -1.6) {$f(2) =$};
\node[above] at (-1.2, -2.4) {$f(3) =$};
\node[above] at (-1.2, -3.2) {$f(4) =$};

% Decimal digits (simplified for illustration)
\foreach \i/\digits in {0/7,3,8,4,1,2,9, 1/3,1,4,1,5,9,2, 2/2,7,1,8,2,8,1, 3/1,4,1,4,2,1,3, 4/6,2,3,4,5,6,7} {
    \foreach \j [count=\k from 1] in \digits {
        \node[number] at (\k*1.2, -\i*0.8) {\j};
    }
}

% Highlight the diagonal
\foreach \i in {0,...,4} {
    \pgfmathtruncatemacro{\col}{\i+1}
    \draw[red, thick, opacity=0.7] (\col*1.2-0.2, -\i*0.8-0.3) -- (\col*1.2+0.2, -\i*0.8+0.3);
}

% The new number r
\node[below, color=blue!70!black, font=\bfseries] at (3.6, -4.2) {$r = 0.$};
\node[below, color=blue!70!black, font=\bfseries] at (0, -4.2) {The new number:};

% Show the diagonal digits
\foreach \i in {0,...,4} {
    \pgfmathtruncatemacro{\col}{\i+1}
    \pgfmathtruncatemacro{\val}{\i==0 ? 3 : (\i==1 ? 7 : (\i==2 ? 3 : (\i==3 ? 7 : 3)))}
    \node[blue!70!black, font=\bfseries] at (\col*1.2, -4.0) {\val};
}

% Arrows showing r differs from each f(n)
\foreach \i in {0,...,4} {
    \pgfmathtruncatemacro{\col}{\i+1}
    \draw[red!60, thick, ->] (\col*1.2, -\i*0.8-0.35) -- (\col*1.2, -3.8);
}

% Caption
\node[below, align=center, text width=8cm] at (3.6, -5.0) {
    \textbf{Cantor's diagonal argument:} The new number $r$ differs from each $f(n)$ in the $n$th decimal place (shown in red), so $r$ is not in the list.
};

\end{tikzpicture}
\caption{Visualization of Cantor's diagonal argument. The decimal expansions of a purported surjection $f\colon\mathbb{N}\to(0,1)$ are arranged in rows. The diagonal digits (red) are used to construct a new number $r$ that differs from each $f(n)$ in the $n$th decimal place, contradicting surjectivity.}
\label{fig:diagonal}
\end{figure}
...
By 1900, Cantor had spent years trying to prove that no such set exists---that is, that $\mathfrak{c} = \aleph_1$, where $\aleph_1$ is the next cardinal after $\aleph_0$. He exchanged letters with Dedekind on the topic and wrote about it extensively, but he never found a proof. The question was wide open when Hilbert placed it first on his list of twenty-three problems.
...
The story of the Continuum Hypothesis is one of the major intellectual developments of the twentieth century. It unfolds in three acts.
...
In 1931, Kurt G\"odel (1906--1978) had already influenced the mathematical community with his \emph{incompleteness theorems}, which showed that any consistent formal system powerful enough to describe arithmetic contains true statements that cannot be proved within the system. This raised a natural question: could the Continuum Hypothesis be one of those undecidable statements?

G\"odel made significant progress in 1940. He introduced a construction called the \textbf{constructible universe}, denoted $L$.
...
This was a significant result. It showed that CH \emph{cannot be disproved} from the axioms of set theory (assuming those axioms are consistent). But it left open the question: could CH be \emph{proved}?
...
Combining G\"odel's and Cohen's results gives a notable conclusion:
...
The resolution of the Continuum Hypothesis is one of the significant results in the foundations of mathematics. But it raises as many questions as it answers.
...
Despite---or perhaps because of---its independence, CH remains one of the central organizing questions of set theory. The search for new axioms that might settle CH has driven some of the major research in the field. Cohen's method of forcing has become one of the most important tools in all of set theory, with applications far beyond the original question.
...
In a sense, Hilbert's first problem was ``resolved''---but the resolution was so unexpected and so rich that it opened up entirely new areas of mathematics. This is perhaps the strongest endorsement one can give to a mathematical problem.

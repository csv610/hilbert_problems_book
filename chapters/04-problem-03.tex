...
\begin{tikzpicture}[
    scale=0.8,
    font=\small,
    piece/.style={draw, fill=blue!20, thick},
    label/.style={font=\footnotesize}
]

% 2D case: Square to Rectangle (works)
\node[above, font=\bfseries] at (2.5, 3.5) {\textbf{2D: Area determines scissors congruence}};

% Square
\draw[piece] (0,0) rectangle (2,2);
\node[label] at (1, 1) {$2 \times 2$};
\node[label] at (1, 0) {Area $= 4$};

% Arrow
\draw[->, thick, color=green!60!black] (2.5, 1) -- (3.5, 1);
\node[label, above] at (3, 1.2) {cut \& reassemble};

% Rectangle
\draw[piece, fill=green!20] (4,0.5) rectangle (6,1.5);
\node[label] at (5, 1) {$4 \times 1$};
\node[label, below] at (5, 0.5) {Area $= 4$};

% Checkmark
\node[green!60!black, font=\large] at (5, 2.2) {$\checkmark$};

% 3D case: Two tetrahedra (fails)
\node[above, font=\bfseries] at (2.5, -1.5) {\textbf{3D: Volume alone does NOT determine scissors congruence}};

% Tetrahedron 1 (regular)
\draw[piece, fill=red!20] (0.5,-4) -- (2.5,-4) -- (1.5,-2.5) -- cycle;
\node[label] at (1.5, -3.8) {$T_1$};
\node[label, below] at (1.5, -4.2) {Volume $= \frac{1}{3}$};

% Arrow
\draw[->, thick, color=red!70] (3, -3.2) -- (4, -3.2);
\node[label, above, color=red!70] at (3.5, -3) {cannot cut \& reassemble!};

% Tetrahedron 2 (irregular)
\draw[piece, fill=red!20] (4.5,-4) -- (6.5,-4) -- (5.5,-2.5) -- cycle;
\node[label] at (5.5, -3.8) {$T_2$};
\node[label, below] at (5.5, -4.2) {Volume $= \frac{1}{3}$};

% X mark
\node[red!70, font=\large] at (5.5, -2.2) {$\times$};

% Dehn invariant note
\node[below, align=center, text width=8cm, color=red!70] at (3.5, -5.0) {
    \textbf{Dehn invariant} differs: $T_1$ has dihedral angles that are rational multiples of $\pi$, while $T_2$ does not.
};

\end{tikzpicture}
\caption{Scissors congruence works in 2D (area is a complete invariant) but fails in 3D. Two tetrahedra with equal volume cannot necessarily be cut and reassembled into each other, as shown by Dehn's invariant.}
\label{fig:scissors}
\end{figure}

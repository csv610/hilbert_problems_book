...
Before we can appreciate the depth of Hilbert's question, we need to understand a hierarchy of ``complexity'' that mathematicians use to classify real (and complex) numbers. This hierarchy is one of the significant ideas in number theory.
...
A real number that is \emph{not} rational is called \textbf{irrational}. The ancient Greeks discovered, to their astonishment, that $\sqrt{2}$ is irrational: there is no fraction equal to it. The proof is a standard argument in mathematics, and we will review it in the exercises below.
...
The transcendental numbers are those real numbers that fall outside the algebraic numbers---the ``gaps'' in the algebraic landscape. For a long time, it was not even clear that transcendental numbers \emph{existed}. The proof that they do was a significant achievement of 19th-century mathematics.
...
The first number proven to be transcendental was $e$ (Euler's number, approximately $2.71828\ldots$). In 1873, Charles Hermite proved that $e$ is transcendental. His proof was a technical achievement in analysis, exploiting the rapidly converging series
\[
    e = \sum_{n=0}^{\infty} \frac{1}{n!}
\]
and the fact that this series converges so quickly that it ``outruns'' any polynomial approximation.

Nine years later, in 1882, Ferdinand Lindemann extended Hermite's methods to prove the transcendence of $\pi$. This result settled one of the oldest open problems in mathematics: the impossibility of \emph{squaring the circle}---constructing, with compass and straightedge alone, a square whose area equals that of a given circle. Since compass-and-straightedge constructions can only produce algebraic numbers, and $\pi$ is transcendental, the task is impossible.

\subsection{Lindemann's Proof That \texorpdfstring{$\pi$}{pi} Is Transcendental}

Lindemann's argument is clear in its broad outlines. We sketch the key idea.
...
In 1934, independently and almost simultaneously, Alexander Gelfond and Theodor Schneider proved one of the fundamental results of 20th-century number theory:
...
This theorem has significant consequences. Here are some of its immediate consequences:
...
Let us spell out the argument for $e^\pi$ in detail, since it is one of the well-known deductions in mathematics.
...
This is a chain of reasoning: we start with the geometry of the circle (Euler's identity connects $e$, $\pi$, and $i$), apply a deep theorem about algebraic exponents, and conclude something specific about a concrete, computable constant.
...
The Gelfond--Schneider theorem was extended by Alan Baker in 1966. Baker proved the following generalization:
...
Despite these results, transcendental number theory remains a field of open problems. Among the most famous:

...
\[
    x^3 + y^3 + z^3 = 33,
\]
can be very hard to analyze: this particular equation was only shown to have the solution $x = 886{,}612{,}897{,}523{,}968{,}960$ in 2019, after years of computational search.
...
The answer to this question is one of the significant results in twentieth-century mathematics.
...
\paragraph{Robinson (1950s--1960s).} Julia Robinson made a contribution by showing that a certain set---the set of numbers that are `` Fibonacci-like'' in a specific technical sense---was Diophantine, assuming the DPRM conjecture. Her work established a connection between the theory of Diophantine equations and the theory of computation.
...
The undecidability of Hilbert's tenth problem is notable for several reasons.
...
The DPRM theorem remains one of the elegant and surprising results in mathematical logic. It has been refined and extended in various directions:

\begin{itemize}
    \item \textbf{Bounds on the polynomial.} Matiyasevich's original proof used polynomials with many variables. Subsequent work has reduced the number of variables needed. It is now known that there exists a single polynomial in 9 variables whose positive values are exactly the set of prime numbers---a fact that would have surprised Diophantus.
...
\begin{solvedbox}
Hilbert's tenth problem is \textbf{solved}. The answer is \emph{negative}: there is no general algorithm that decides, given a Diophantine equation, whether it has integer solutions. This was proved by Yuri Matiyasevich in 1970, completing work of Davis, Putnam, and Robinson. The key result---the DPRM theorem---establishes that every recursively enumerable set is Diophantine, linking number theory to computability theory.
\end{solvedbox}
